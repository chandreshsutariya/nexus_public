    ...
    try:
        ...
        chat_completion = client.chat.completions.create(
            messages=[
                ...
                {"role": "user", "content": f"Project Description: {body.project_id}. \
...
async def task_assistant(body: TaskDescription):
    try:
        ...
        chat_completion = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "You are a helpful assistant that provides guidance for project."},
                {"role": "user", "content": f"Given the project description: {body.project_id}, and the \
...
async def module_assistant(body: ModuleRequest):
    try:
        ...
        chat_completion = client.chat.completions.create(
            messages=[
                {"role": "system", "content": ""},
                {"role": "user", "content": f"Using the project example context: {body.project_context},  suggest technology stack."}
            ],
            model="gpt-4o"
        )
        modules = chat_completion.choices[0].message.content
